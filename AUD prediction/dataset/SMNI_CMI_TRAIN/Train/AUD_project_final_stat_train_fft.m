 ...
 for i=1:numexcelfiles    
     ...
     for ii=1:5
         ...
         x=x+1;
%          signal(i,x)=var(wave(:,ii));  %it is 0
%          x=x+1;
%          signal(i,x)=std(wave(:,ii));  %it is 0
%          x=x+1;
         signal(i,x)=max(wave(:,ii));
         x=x+1;
         signal(i,x)=min(wave(:,ii));
         x=x+1;
%          signal(i,x)=kurtosis(wave(:,ii));  %cannot be measured
%          x=x+1;
%          signal(i,x)=skewness(wave(:,ii));  %cannot be measured
%          x=x+1;
     end 
     yy=1;
     x=1;
   ...
   filename='F:\AUD files for LSTM\MTech project final susma\statistical coeff\fft\labels train fft.xlsx';
